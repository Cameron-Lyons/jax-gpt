...
class BenchmarkRunner:
    ...
    def setup_synthetic_data(self):
        ...
        self.train_data = random.randint(
            data_rng, shape=(1000000,), minval=0, maxval=self.config.vocab_size, dtype=jnp.uint16
        )
    ...
    def create_training_step(self):
        ...
        def train_step(params, opt_state, batch):
            (loss, logits), grads = value_and_grad(loss_fn, has_aux=True)(params, batch)
    ...
    def benchmark_forward_pass(self) -> Dict[str, Any]:
        ...
        for run in range(self.config.num_runs):
            for _ in range(self.config.warmup_steps):
                ...
                out = self.model.apply({"params": self.variables["params"]}, batch[0])
        ...
        if self.config.verbose:
            print(
                f"Latency: {results['latency_ms']['mean']:.2f}"
                f" ± {results['latency_ms']['std']:.2f} ms"
            )
    ...
    def benchmark_memory_usage(self) -> Dict[str, Any]:
        ...
        if self.config.verbose:
            baseline = results["baseline_memory_mb"]
            peak = results["peak_memory_mb"]
            delta = results["memory_delta_mb"]
            if baseline is not None:
                print(f"Baseline memory: {baseline:.2f} MB")
            if peak is not None:
                print(f"Peak memory: {peak:.2f} MB")
            assert isinstance(delta, dict)
            print(f"Memory delta: {delta['mean']:.2f} ± {delta['std']:.2f} MB")

        return results

    def calculate_mfu(self, throughput: float, forward_only: bool = False) -> float:
        """Calculate Model FLOPs Utilization (MFU).

        Uses the standard approximation: 6*N FLOPs per token for a training
        step (2*N forward + 4*N backward), or 2*N for forward-only inference.
        """
        n_params = self.count_parameters()
        seq_length = self.config.block_size
        flops_per_token = 2 * n_params if forward_only else 6 * n_params
        tokens_per_sec = throughput * seq_length
    ...
    def run_comprehensive_benchmark(self) -> Dict[str, Any]:
        ...
        if self.config.measure_mfu:
            forward_mfu = self.calculate_mfu(
                forward_results["throughput_samples_per_sec"]["mean"],
                forward_only=True,
            )
            training_mfu = self.calculate_mfu(
                training_results["throughput_samples_per_sec"]["mean"],
                forward_only=False,
            )

            mfu_results = {"forward_mfu": forward_mfu, "training_mfu": training_mfu}
...
def main():
    ...
    parser.add_argument("--compile", action="store_true", default=True, help="Use JIT compilation")
    parser.add_argument(
        "--no_compile", dest="compile", action="store_false", help="Disable JIT compilation"
    )

    parser.add_argument("--warmup_steps", type=int, default=10, help="Number of warmup steps")
    parser.add_argument("--benchmark_steps", type=int, default=50, help="Number of benchmark steps")
    ...
    parser.add_argument(
        "--use_real_data", action="store_true", default=True, help="Use real data for benchmarking"
    )
    parser.add_argument(
        "--use_synthetic_data",
        dest="use_real_data",
    ...
    parser.add_argument(
        "--output_dir", type=str, default="benchmarks", help="Directory to save benchmark results"
    )
    parser.add_argument(
        "--no_save", dest="save_results", action="store_false", help="Don't save benchmark results"
    )
    ...
    if config.verbose:
        ...
        fwd_tp = results["forward_pass"]["throughput_samples_per_sec"]["mean"]
        train_tp = results["training_step"]["throughput_samples_per_sec"]["mean"]
        print(f"Forward throughput: {fwd_tp:.2f} samples/sec")
        print(f"Training throughput: {train_tp:.2f} samples/sec")
